...
const name = $("quickName").value.trim();
      ...
      const card = {
        ...
        cardmarketProductId: $("quickMarketProductId") ? $("quickMarketProductId").value.trim() : "",
    ...
    function validateCollectionIdField(fieldId = "cardId") {
      const field = $(fieldId);
      if (!field) return true;

      field.value = normalizeCollectionIdValue(field.value);
      if (!/^\d{6}$/.test(field.value)) {
        field.focus();
        showImportToast("Sammlungs-ID ungültig", "Die Sammlungs-ID muss genau aus 6 Ziffern bestehen.", "error");
        return false;
      }
    ...
    function generateSixDigitCollectionId() {
...
function goToPageFile(pageId) {
    ...
    function resetForm() {
      ...
      if ($("cardId")) $("cardId").value = generateSixDigitCollectionId();
...
const existingEditId = cv258CurrentEditId();
      ...
      const now = new Date().toISOString();
      const oldCard = cv258FindCard(existingEditId || id);
      ...
      const card = {
        ...
        name,
        number,
        apiCardId: selectedApiCard ? (selectedApiCard.apiCardId || "") : ((oldCard && oldCard.apiCardId) || ""),
        apiImage: selectedApiCard ? (selectedApiCard.image || "") : ((oldCard && oldCard.apiImage) || ""),
        apiVariant: selectedApiCard ? (selectedApiCard.variant || "") : ((oldCard && oldCard.apiVariant) || ""),
        marketProductId: valueOf("cardmarketProductId").trim(),
        cardmarketProductId: valueOf("cardmarketProductId").trim(),
    ...
    function ensureConditionOption(value) {
      const select = $("cardCondition");
      const condition = String(value || "").trim();
      if (!select || !condition) return;
      const exists = Array.from(select.options).some(option => option.value === condition);
      if (!exists) {
...
function setValueIfExists(id, value) {
    ...
    function galleryImageSources(card) {
      ...
      const savedApiImage = String((card && card.apiImage) || "").trim();
      const apiSources = onePieceApiImageCandidates(enrichedCard).filter(Boolean);

      const ownImages = Array.isArray(card && card.images) ? card.images.filter(Boolean) : [];
      const ownFront = ownImages[0] || "";
      const ownRest = ownImages.slice(1);

      const ordered = []
        .concat(savedApiImage ? [savedApiImage] : [])
        .concat(apiSources)
        .concat(ownFront ? [ownFront] : [])
        .concat(ownRest);
...
<div class="card-title-line">
    ...
    const OPTCG_OPEN_API_BASE_URL = "https://optcgapi.com/api";
    let optcgApiCardsCache = null;
    let selectedApiCard = null;
    let apiInlineSearchTimer = null;
    ...
    function firstPresent(...values) {
      return values.find(value => value !== undefined && value !== null && String(value).trim() !== "") || "";
        ...
        function normalizeApiSetCode(value) {
      const text = String(value || "").toUpperCase().trim();
      const match = text.match(/\b(OP|ST|EB|PRB)[-\s]?(\d{1,3})\b/i);
      if (!match) return "";
      return match[1].toUpperCase() + match[2].padStart(2, "0");
    }

    function extractDeepValues(obj, depth = 0) {
      ...
      Object.entries(obj).forEach(([key, value]) => {
        ...
        if (typeof value === "string" || typeof value === "number") {
          ...
          values.push(...extractDeepValues(value, depth + 1));
        }
      });
      return values;
    }

    function extractApiCardNumber(raw) {
      if (!raw) return "";

      const deepText = extractDeepValues(raw).join(" ");
      const fullMatch = deepText.match(/(?:OP|ST|EB|PRB|P)[-\s]?\d{1,3}[-\s]?\d{3}/i);
      if (fullMatch) return normalizeOnePieceCardNumber(fullMatch[0]) || fullMatch[0].toUpperCase().replace(/\s+/g, "");

      // Auch Bild-URLs prüfen, z. B. .../OP01-001.png oder .../OP-01_001.webp
      const imageText = [
        raw.image_url,
        raw.image,
        raw.imageUrl,
        raw.card_image,
        raw.images && raw.images.small,
        raw.images && raw.images.large,
        raw.images && raw.images.full,
        raw.assets && raw.assets.image,
        raw.picture,
        raw.url
      ].filter(Boolean).join(" ");
      const imageMatch = imageText.match(/(?:OP|ST|EB|PRB|P)[-\s_]?(\d{1,3})[-\s_]?(\d{3})/i);
      if (imageMatch) {
        const prefix = imageMatch[0].match(/^(OP|ST|EB|PRB|P)/i)[1].toUpperCase();
        if (prefix === "P") return "P-" + imageMatch[2].padStart(3, "0");
        return prefix + imageMatch[1].padStart(2, "0") + "-" + imageMatch[2].padStart(3, "0");
      }
      ...
      const setCandidates = [
        ...
        raw.product_set,
        raw.productSet,
        raw.card && raw.card.set_id,
        raw.card && raw.card.setId,
        raw.card && raw.card.set_code,
        raw.card && raw.card.setCode,
        raw.card && raw.card.set,
        raw.set && raw.set.id,
        raw.set && raw.set.code,
        raw.set && raw.set.name,
        raw.expansion && raw.expansion.id,
        raw.expansion && raw.expansion.code,
        raw.expansion && raw.expansion.name
      ];
      ...
      const numberCandidates = [
        ...
        raw.cardId,
        raw.code,
        raw.card && raw.card.number,
        raw.card && raw.card.card_number,
        raw.card && raw.card.cardNumber,
        raw.card && raw.card.localId,
        raw.card && raw.card.local_id,
        raw.card && raw.card.id,
        raw.card && raw.card.code
      ];

      const setCode = setCandidates.map(normalizeApiSetCode).find(Boolean);
      const numberText = numberCandidates
        .filter(value => value !== undefined && value !== null && String(value).trim() !== "")
        .join(" ");

      const numberMatch = numberText.match(/\b(\d{3})\b/) || numberText.match(/(\d{1,3})$/);
      if (setCode && numberMatch) {
        return setCode + "-" + numberMatch[1].padStart(3, "0");
      }

      // Falls alles in einem id/code steckt, z. B. OP-01_001
      const loose = [
        raw.id,
        raw.code,
        raw.card_id,
      ...
      const looseMatch = loose.match(/\b(OP|ST|EB|PRB)[-\s_]?(\d{1,3})[-\s_]?(\d{3})\b/i);
      if (looseMatch) {
        return looseMatch[1].toUpperCase() + looseMatch[2].padStart(2, "0") + "-" + looseMatch[3].padStart(3, "0");
      }
    ...
    function normalizeApiCard(raw) {
      ...
      const extractedNumber = extractApiCardNumber(raw);
      const id = firstPresent(raw.id, raw.card_id, raw.cardId, raw.code, extractedNumber, raw.number, raw.card_number, raw.cardNumber);
      const name = firstPresent(raw.name, raw.card_name, raw.cardName, raw.en_name, raw.english_name);
      const number = firstPresent(extractedNumber, raw.number, raw.card_number, raw.cardNumber, raw.card_no, raw.cardNo, raw.code);
      const setName = firstPresent(raw.set, raw.set_name, raw.setName, raw.expansion, raw.pack, raw.booster, raw.deck, raw.set && raw.set.name, raw.expansion && raw.expansion.name);
      const setId = firstPresent(raw.set_id, raw.setId, raw.set_code, raw.setCode, raw.set && raw.set.code, raw.set && raw.set.id);
      const rarity = firstPresent(raw.rarity, raw.rarity_name, raw.rarityName);
      const variant = firstPresent(raw.variant_type, raw.variant, raw.parallel ? "Parallel" : "", raw.is_parallel ? "Parallel" : "");
      const apiImage = firstPresent(raw.image_url, raw.image, raw.images && raw.images.small, raw.images && raw.images.large, raw.images && raw.images.full, raw.card_image, raw.imageUrl);

      const normalizedNumber = normalizeOnePieceCardNumber(number || extractedNumber || id);
      const image = apiImage || (normalizedNumber ? (OPTCG_IMAGE_BASE_URL + encodeURIComponent(normalizedNumber)) : "");

      return {
        raw,
        apiCardId: String(id || normalizedNumber || "").trim(),
        name: String(name || "").trim(),
        number: String(normalizedNumber || extractedNumber || number || "").trim(),
        set: String(setName || setId || "").trim(),
        rarity: String(rarity || "").trim(),
        variant: String(variant || "").trim(),
        parallel: !!(raw.parallel || raw.is_parallel || variant),
        image
      };
    }

    function flattenApiCards(payload) {
    ...
    async function loadOptcgApiCards() {
      if (optcgApiCardsCache) return optcgApiCardsCache;
      const endpoints = [
        OPTCG_OPEN_API_BASE_URL + "/allSetCards/",
        OPTCG_OPEN_API_BASE_URL + "/allSTCards/",
        OPTCG_OPEN_API_BASE_URL + "/allPromoCards/"
      ];
      const batches = await Promise.allSettled(endpoints.map(fetchJsonSafe));
      const cards = [];
      batches.forEach(result => {
        if (result.status !== "fulfilled") return;
        flattenApiCards(result.value).forEach(card => {
          const normalized = normalizeApiCard(card);
          if (normalized && normalized.name) cards.push(normalized);
        });
      });
      optcgApiCardsCache = cards;
      return cards;
    }

    function scoreApiCard(card, query) {
      ...
      let score = 0;
      if (number === q) score += 1000;
      if (number.includes(q)) score += 600;
      if (name === q) score += 500;
      if (name.includes(q)) score += 300;
      if (set.includes(q)) score += 80;
      if (card.variant || card.parallel) score += 15;
      return score;
    }

    function renderApiCardResults(cards) {
      const target = $("apiCardResults");
      if (!target) return;
      if (!cards.length) {
        target.innerHTML = '<div class="catalog-empty">Keine API-Karte gefunden.</div>';
        return;
      }
      target.innerHTML = cards.slice(0, 30).map((card, index) => `
        <button class="api-card-result" type="button" data-api-card-index="${index}">
          <div class="api-card-thumb">${card.image ? `<img src="${escapeHtml(card.image)}" alt="${escapeHtml(card.name)}">` : "CARD"}</div>
          <div class="api-card-result-main">
            <div class="api-card-result-name">${escapeHtml(card.name)}</div>
            <div class="api-card-result-meta">${escapeHtml([card.number, card.set, card.rarity, card.variant].filter(Boolean).join(" · "))}</div>
          </div>
        </button>
      `).join("");
      target.querySelectorAll("[data-api-card-index]").forEach(button => {
        button.addEventListener("click", () => pickApiCard(cards[Number(button.dataset.apiCardIndex)]));
      });
    }


    function renderInlineApiCardSuggestions(cards) {
      const box = $("inlineApiCardSuggestions");
      const list = $("inlineApiCardSuggestionList");
      const count = $("inlineApiSuggestionCount");
      if (!box || !list) return;

      if (count) count.textContent = cards.length + " Treffer";
      ...
      list.innerHTML = cards.map((card, index) => `
        <button class="inline-api-card" type="button" data-inline-api-card-index="${index}">
          <div class="api-card-thumb">${card.image ? `<img src="${escapeHtml(card.image)}" alt="${escapeHtml(card.name)}">` : "CARD"}</div>
          <div class="inline-suggestion-main">
            <div class="inline-suggestion-name">${escapeHtml(card.name)}</div>
            <div class="inline-suggestion-meta">${escapeHtml([card.number, card.set, card.rarity, card.variant].filter(Boolean).join(" · "))}</div>
          </div>
        </button>
      `).join("");

      list.querySelectorAll("[data-inline-api-card-index]").forEach(button => {
        button.addEventListener("click", () => {
          const card = cards[Number(button.dataset.inlineApiCardIndex)];
          try { pickApiCard(card); } catch (error) { console.error("pickApiCard failed", error); showImportToast("Fehler", "Die API-Karte konnte nicht übernommen werden.", "error"); }
        });
      });
    }

    async function updateInlineApiSuggestions() {
      const name = $("cardName") ? $("cardName").value.trim() : "";
      const number = $("cardNumber") ? $("cardNumber").value.trim() : "";
      const query = number || name;
      const status = $("inlineApiSearchStatus");

      if (!query || query.length < 2) {
        renderInlineApiCardSuggestions([]);
        if (status) status.textContent = "";
        return;
      }

      if (status) status.textContent = "API wird durchsucht …";

      try {
        const allCards = await loadOptcgApiCards();
        const normalizedNumberQuery = normalizeOnePieceCardNumber(number || "");
        let results = [];

        if (normalizedNumberQuery) {
          results = allCards.filter(card => {
            const cardNumber = normalizeOnePieceCardNumber(card.number || "");
            return cardNumber && cardNumber === normalizedNumberQuery;
          });
        } else {
          const normalizedNameQuery = String(name || "").toLowerCase().trim();
          results = allCards
            .map(card => ({ card, score: scoreApiCard(card, normalizedNameQuery) }))
        ...
        if (status) {
          status.textContent = results.length
            ? (results.length + " API-Varianten gefunden.")
            : "Keine API-Treffer gefunden.";
        }

        renderInlineApiCardSuggestions(results);
      } catch (error) {
        console.error(error);
        if (status) status.textContent = "API-Suche konnte nicht geladen werden.";
        renderInlineApiCardSuggestions([]);
      }
    }

    function scheduleInlineApiSuggestions() {
      clearTimeout(apiInlineSearchTimer);
      apiInlineSearchTimer = setTimeout(updateInlineApiSuggestions, 350);
    }

    async function searchApiCards() {
      const input = $("apiCardSearchInput");
      const status = $("apiCardSearchStatus");
      const query = input ? input.value.trim() : "";
      if (!query) {
        if (status) status.textContent = "Bitte Name oder Kartennummer eingeben.";
        renderApiCardResults([]);
        return;
      }
      if (status) status.textContent = "API-Karten werden geladen …";
      try {
        const allCards = await loadOptcgApiCards();
        const results = allCards
          .map(card => ({ card, score: scoreApiCard(card, query) }))
          .filter(item => item.score > 0)
          .sort((a, b) => b.score - a.score)
          .map(item => item.card);
        if (status) status.textContent = results.length ? (results.length + " API-Treffer gefunden.") : "Keine API-Treffer gefunden.";
        renderApiCardResults(results);
      } catch (error) {
        console.error(error);
        if (status) status.textContent = "API-Suche konnte nicht geladen werden. Prüfe Internet/CORS.";
        renderApiCardResults([]);
      }
    }

    function findBestCardmarketProductsForApiCard(apiCard, limit = 8) {
      if (!apiCard || !Array.isArray(cardmarketProducts)) return [];
      const normalizedNumber = normalizeOnePieceCardNumber(apiCard.number);
      const nameWords = apiCard.name.toLowerCase().split(/\s+/).filter(Boolean);
      const scored = cardmarketProducts.map(item => {
        const product = item.product || item;
        const haystack = [product.name, product.enName, product.productName, product.baseName, product.expansionName, product.categoryName, product.number].filter(Boolean).join(" ").toLowerCase();
        let score = 0;
        if (normalizedNumber && haystack.includes(normalizedNumber.toLowerCase())) score += 800;
        nameWords.forEach(word => { if (word.length > 2 && haystack.includes(word)) score += 80; });
        if (apiCard.variant || apiCard.parallel) {
          if (/parallel|alternate|alternative|alt\s*art|manga|promo|winner|championship|special/i.test(haystack)) score += 160;
        }
        if (apiCard.set && haystack.includes(apiCard.set.toLowerCase())) score += 80;
        if (apiCard.rarity && haystack.includes(apiCard.rarity.toLowerCase())) score += 30;
    ...
    function getApiCardNumberForForm(apiCard) {
      if (!apiCard) return "";

      const direct = normalizeOnePieceCardNumber(apiCard.number || apiCard.apiCardId || "");
      if (direct) return direct;

      const raw = apiCard.raw || {};
      const extracted = typeof extractApiCardNumber === "function" ? extractApiCardNumber(raw) : "";
      const normalizedExtracted = normalizeOnePieceCardNumber(extracted);
      ...
      const match = candidates.match(/(?:OP|ST|EB|PRB|P)-?\s?\d{2,3}-?\d{3}/i);
      if (match) return normalizeOnePieceCardNumber(match[0]) || match[0].toUpperCase().replace(/\s+/g, "");

      const set = String(raw.set_id || raw.setId || raw.set_code || raw.setCode || apiCard.set || "").toUpperCase();
      const numberOnly = String(raw.number || raw.card_number || raw.cardNumber || apiCard.number || "").trim();
      const setMatch = set.match(/(?:OP|ST|EB|PRB)\d{2,3}/i);
      const numMatch = numberOnly.match(/\d{3}/);
      if (setMatch && numMatch) return (setMatch[0] + "-" + numMatch[0]).toUpperCase();

      return apiCard.number || "";
    }

    function setFormCardNumberFromApi(apiCard) {
      const number = getApiCardNumberForForm(apiCard);
    ...
    function setVisibleCardNumberField(value) {
      const number = String(value || "").trim();
      const fields = Array.from(document.querySelectorAll('#cardNumber, input[name="cardNumber"], input[data-card-number]'));
    ...
    function resolveApiCardNumberForForm(apiCard) {
      if (!apiCard) return "";

      const direct = normalizeOnePieceCardNumber(apiCard.number || "");
      if (direct) return direct;

      const raw = apiCard.raw || {};
      const extracted = extractApiCardNumber(raw);
      const normalizedExtracted = normalizeOnePieceCardNumber(extracted);
      if (normalizedExtracted) return normalizedExtracted || extracted;

      const fromId = normalizeOnePieceCardNumber(apiCard.apiCardId || "");
      if (fromId) return fromId;

      return extracted || apiCard.number || "";
    }

    function safeApiCardNumberForPick(apiCard) {
      try {
        if (typeof resolveApiCardNumberForForm === "function") {
          const resolved = resolveApiCardNumberForForm(apiCard);
          if (resolved) return resolved;
        }
      } catch (error) {
        console.warn("resolveApiCardNumberForForm failed", error);
      }

      try {
        if (typeof normalizeOnePieceCardNumber === "function") {
          const normalized = normalizeOnePieceCardNumber((apiCard && (apiCard.number || apiCard.apiCardId)) || "");
          if (normalized) return normalized;
        }
      } catch (error) {
        console.warn("normalizeOnePieceCardNumber failed", error);
      }

      return String((apiCard && apiCard.number) || "").trim();
    }

    function safeSetVisibleCardNumber(value) {
      const number = String(value || "").trim();
      const field = $("cardNumber");
      if (field) {
        field.value = number;
    ...
    function pickApiCard(apiCard) {
      if (!apiCard) return;

      selectedApiCard = apiCard;

      const pickedNumber = safeApiCardNumberForPick(apiCard);
      const pickedName = String(apiCard.name || "").trim();
      const pickedSet = String(apiCard.set || "").trim();
      const pickedRarity = String(apiCard.rarity || "").trim();
      const pickedVariant = String(apiCard.variant || "").trim();

      setValueIfExists("cardName", pickedName);
      safeSetVisibleCardNumber(pickedNumber);
      setValueIfExists("cardSet", pickedSet);
      if ($("cardRarity")) $("cardRarity").value = pickedRarity;

      requestAnimationFrame(() => safeSetVisibleCardNumber(pickedNumber));

      if ($("cardNotes")) {
        const existing = $("cardNotes").value.trim();
        const apiNote = "API-Karte: " + [apiCard.apiCardId || "", pickedVariant].filter(Boolean).join(" · ");
        $("cardNotes").value = existing ? (existing + "\n" + apiNote) : apiNote;
      }

      // API-Bild wird separat gespeichert und nur für die Galerie genutzt.
      // Es wird nicht als eigenes Detailfoto/frontImage übernommen.

      const meta = [pickedNumber || apiCard.number || "", pickedSet, pickedRarity, pickedVariant].filter(Boolean).join(" · ");
      const selectedHtml = `
        <div class="api-selected-card">
          <div class="api-card-thumb">${apiCard.image ? `<img src="${escapeHtml(apiCard.image)}" alt="${escapeHtml(pickedName || "Karte")}">` : "CARD"}</div>
          <div>
            <strong>${escapeHtml(pickedName || "Ausgewählte Karte")}</strong>
            <div class="catalog-result-meta">${escapeHtml(meta)}</div>
          </div>
        </div>
      `;

      if ($("inlineApiSelectedCard")) {
        $("inlineApiSelectedCard").classList.add("active");
        $("inlineApiSelectedCard").innerHTML = selectedHtml;
      }

      if ($("inlineApiCardSuggestions")) $("inlineApiCardSuggestions").classList.add("active");
      if ($("inlineApiCardSuggestionList")) $("inlineApiCardSuggestionList").innerHTML = "";

      if ($("apiCardSelectedBox")) {
        $("apiCardSelectedBox").classList.add("active");
        $("apiCardSelectedBox").innerHTML = selectedHtml;
      }

      const matches = findBestCardmarketProductsForApiCard(apiCard);
      if (matches.length) renderCatalogResults(matches.map(product => ({ product, score: 1 })), "full");

      showImportToast("API-Karte übernommen", "Kartendaten und Bild wurden aus der API übernommen. Cardmarket-Treffer wurden vorgeschlagen.", "success");
    }
    ...
    function pickCatalogProduct(productId, mode = "full") {
      ...
      cv220SelectCardmarketProduct(product);
    ...
    function importCardmarketPriceFile(file) {
      ...
      cv220RefreshCardmarketPrices({ silent: false });
    ...
    /* v220: robuster Cardmarket-Diagnose- und Preisabgleich */
    function cv220NormalizeId(value) {
      return String(value || "").trim().replace(/^0+(\d)/, "$1");
    }

    function cv220ProductIdFromCard(card) {
      if (!card) return "";
      return cv220NormalizeId(card.cardmarketProductId || card.marketProductId || card.idProduct || card.productId || card.id_product || "");
    }

    function cv220ProductIdFromProduct(product) {
      if (!product) return "";
      return cv220NormalizeId(product.idProduct || product.productId || product.cardmarketProductId || product.id || product.id_product || "");
    }

    function cv220ProductIdFromPrice(price) {
      if (!price) return "";
      return cv220NormalizeId(price.idProduct || price.productId || price.cardmarketProductId || price.id || price.id_product || "");
    }

    function cv220ExtractPrice(price) {
      if (!price) return "";
      const candidates = [price.trend, price.avg, price.avg30, price.avg7, price.low, price.lowest, price.marketPrice, price.price, price.value, price.trendPrice, price.averageSellPrice, price.lowPrice, price.avgSell, price.avgSellPrice];
      for (const candidate of candidates) {
        if (candidate === undefined || candidate === null || candidate === "") continue;
        const number = Number(String(candidate).replace("€", "").replace(/\s/g, "").replace(",", ".").trim());
        if (Number.isFinite(number) && number >= 0) return number;
      }
      return "";
    }

    function cv220FormatEuro(value) {
      const number = Number(value);
      if (!Number.isFinite(number)) return "";
      return number.toLocaleString("de-DE", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + " €";
    }

    function cv220MirrorProductIdsOnCards() {
      let changed = false;
      cards = (cards || []).map(card => {
        const id = cv220ProductIdFromCard(card);
        if (!id) return card;
        if (card.cardmarketProductId === id && card.marketProductId === id) return card;
        changed = true;
        return { ...card, cardmarketProductId: id, marketProductId: id };
      });
      if (changed) saveCards();
      return changed;
    }

    function cv220RefreshCardmarketPrices(options = {}) {
      const silent = options.silent !== false;
      if (!Array.isArray(cards)) cards = [];
      if (!Array.isArray(cardmarketPrices)) cardmarketPrices = [];

      cv220MirrorProductIdsOnCards();
      const cardsWithIds = cards.filter(card => cv220ProductIdFromCard(card)).length;

      if (!cards.length || !cardmarketPrices.length) {
        if (!silent && typeof showImportToast === "function") {
          showImportToast("Cardmarket nicht geladen", "Preisdatensätze: " + cardmarketPrices.length + " · Karten mit Product-ID: " + cardsWithIds, "warning");
        }
        return { updated: 0, matched: 0, total: cards.length, cardsWithIds, priceRows: cardmarketPrices.length };
      }

      const priceByProductId = new Map();
      cardmarketPrices.forEach(price => {
        const id = cv220ProductIdFromPrice(price);
        if (id && !priceByProductId.has(id)) priceByProductId.set(id, price);
      });
      ...
      cards = cards.map(card => {
        ...
        if (!price) return card;
        const rawPrice = cv220ExtractPrice(price);
        if (rawPrice === "") return card;

        matched += 1;
        const formatted = cv220FormatEuro(rawPrice);
        const changed = String(card.value || "") !== formatted || String(card.marketPrice || "") !== formatted;
      ...
      if (!silent && typeof showImportToast === "function") {
        if (matched) showImportToast("Preise aktualisiert", matched + " von " + cardsWithIds + " Karten mit Product-ID wurden abgeglichen.", "success");
        else showImportToast("Keine Preiszuordnung", cardsWithIds + " Karten haben eine Product-ID, aber kein passender Preis wurde gefunden.", "warning");
      }

      return { updated, matched, total: cards.length, cardsWithIds, priceRows: cardmarketPrices.length };
    }

    function cv220ShowCardmarketDebug() {
      const result = cv220RefreshCardmarketPrices({ silent: true });
      alert("Karten gesamt: " + result.total + "\nKarten mit Product-ID: " + result.cardsWithIds + "\nCardmarket-Preisdatensätze: " + result.priceRows + "\nPreis-Matches: " + result.matched + "\nAktualisiert: " + result.updated);
      return result;
    }

    function cv220SelectCardmarketProduct(product) {
      selectedCatalogProduct = product || null;
      const productId = cv220ProductIdFromProduct(product);
      if ($("cardmarketProductId")) $("cardmarketProductId").value = productId;
      if ($("cardValue") && productId) {
        const price = (cardmarketPrices || []).find(row => cv220ProductIdFromPrice(row) === productId);
        const raw = cv220ExtractPrice(price);
        if (raw !== "") $("cardValue").value = cv220FormatEuro(raw);
      }
      if (typeof renderInlineCatalogSelection === "function") renderInlineCatalogSelection(selectedCatalogProduct);
      return productId;
    }

    window.cardVaultDebugCardmarket = cv220ShowCardmarketDebug;
    refreshCardmarketPricesForCollection = cv220RefreshCardmarketPrices;



    
    /* v222: Öffentliche Sammlung robust laden */
    let publicCollectionLoaded = false;
    let publicCollectionMode = false;

    function publicCollectionCandidateUrls() {
      const urls = [];
      urls.push(new URL("data/collection.json", document.baseURI).href);
      const parts = location.pathname.split("/").filter(Boolean);
      if (parts.length) urls.push(location.origin + "/" + parts[0] + "/data/collection.json");
      urls.push(location.origin + "/CardVault/data/collection.json");
      return Array.from(new Set(urls));
    }

    function normalizePublicCollectionPayload(payload) {
      if (Array.isArray(payload)) return payload;
    ...
    function normalizePublicCard(card) {
      ...
      return {
        ...card,
        id: String(card.id || card.collectionId || card.cardId || Date.now().toString(36) + Math.random().toString(36).slice(2, 7)),
        name: String(card.name || card.cardName || "").trim(),
        number: String(card.number || card.cardNumber || "").trim(),
...
function setPublicCollectionStatus(message, state = "") {
    ...
    async function fetchPublicCollectionJson() {
      let lastError = "";
      for (const baseUrl of publicCollectionCandidateUrls()) {
        const url = baseUrl + (baseUrl.includes("?") ? "&" : "?") + "v=" + Date.now();
        try {
          const response = await fetch(url, { cache: "no-store" });
          if (!response.ok) {
            lastError = "HTTP " + response.status + " bei " + baseUrl;
            continue;
          }
          return { payload: await response.json(), url: baseUrl };
        } catch (error) {
    ...
    async function loadPublicCollectionIfAvailable(options = {}) {
      ...
      setPublicCollectionStatus("Öffentliche Sammlung wird geladen …");
    ...
    function exportPublicCollectionJson() {
      ...
      a.remove();
      URL.revokeObjectURL(url);
      if (typeof showImportToast === "function") showImportToast("collection.json erstellt", "Diese Datei bei GitHub unter data/collection.json hochladen.", "success");
    }
...
function forceMobileCollectionFiltersState() {
    ...
    /* v258: Bearbeiten minimal repariert, Formular-Befüllung bleibt original */
    function cv258LocalCardsRaw() {
      try {
        const raw = localStorage.getItem(STORAGE_KEY);
        const parsed = raw ? JSON.parse(raw) : [];
        return Array.isArray(parsed) ? parsed : [];
    ...
    function cv258SameCard(a, b) {
      const aKeys = new Set(cv258CardKeys(a));
      return cv258CardKeys(b).some(key => aKeys.has(key));
    }

    function cv258FindCard(id) {
      const searchId = String(id || "").trim();
      if (!searchId) return null;
      return (cards || []).find(card => String(card.id) === searchId)
        || cv258LocalCardsRaw().find(card => String(card.id) === searchId)
        || null;
    }

    function cv258SetEditId(id) {
...
document.addEventListener("DOMContentLoaded", () => {
      ...
      setTimeout(() => cv220RefreshCardmarketPrices({ silent: true }), 600);
      const collectionIdInput = $("cardId");
      if (collectionIdInput) {
        collectionIdInput.setAttribute("maxlength", "6");
        collectionIdInput.setAttribute("pattern", "\\d{6}");
        collectionIdInput.setAttribute("inputmode", "numeric");
        collectionIdInput.addEventListener("input", () => {
          collectionIdInput.value = normalizeCollectionIdValue(collectionIdInput.value);
        });
      }

      if ($("cardName")) onIfExists("cardName", "input", scheduleInlineApiSuggestions);
      if ($("cardNumber")) onIfExists("cardNumber", "input", scheduleInlineApiSuggestions);

      if ($("apiCardSearchBtn")) onIfExists("apiCardSearchBtn", "click", searchApiCards);
      if ($("apiCardSearchInput")) onIfExists("apiCardSearchInput", "keydown", event => {
        if (event.key === "Enter") {
          event.preventDefault();
          searchApiCards();
        }
