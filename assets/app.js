...
function setValueIfExists(id, value) {
    ...
    function galleryImageSources(card) {
      ...
      const savedApiImage = card.apiImage || "";
      const apiSources = onePieceApiImageCandidates(enrichedCard);
      const ownFront = (card.images || [])[0] || "";
      return (savedApiImage ? [savedApiImage] : []).concat(apiSources).concat(ownFront ? [ownFront] : []).filter(Boolean);
    }
...
function pickApiCard(apiCard) {
      ...
      setValueIfExists("cardName", apiCard.name);
      const pickedNumber = resolveApiCardNumberForForm(apiCard);
      setVisibleCardNumberField(pickedNumber);
      if (!pickedNumber) console.warn("Keine Kartennummer aus API-Karte erkannt", apiCard);
      setValueIfExists("cardSet", apiCard.set);
      requestAnimationFrame(() => setVisibleCardNumberField(pickedNumber));
      if ($("cardRarity")) $("cardRarity").value = apiCard.rarity || "";
      if ($("cardNotes")) {
        const existing = $("cardNotes").value.trim();
        const apiNote = "API-Karte: " + [apiCard.apiCardId, apiCard.variant || ""].filter(Boolean).join(" · ");
        $("cardNotes").value = existing ? (existing + "\\n" + apiNote) : apiNote;
      }
      if (apiCard.image) {
        frontImage = apiCard.image;
        refreshPreviews();
      }
      if ($("inlineApiSelectedCard")) {
        $("inlineApiSelectedCard").classList.add("active");
        $("inlineApiSelectedCard").innerHTML = `
          <div class="api-selected-card">
            <div class="api-card-thumb">${apiCard.image ? `<img src="${escapeHtml(apiCard.image)}" alt="${escapeHtml(apiCard.name)}">` : "CARD"}</div>
            <div>
              <strong>${escapeHtml(apiCard.name)}</strong>
              <div class="catalog-result-meta">${escapeHtml([pickedNumber || apiCard.number, apiCard.set, apiCard.rarity, apiCard.variant].filter(Boolean).join(" · "))}</div>
            </div>
          </div>
        `;
      }
