...
function validateGradeField(fieldId) {
    ...
    function currentEditingCardForSave() {
      if (!editingId) return null;
      return cards.find(card => String(card.id) === String(editingId)) || null;
    }
...
function setValueIfExists(id, value) {
    ...
    function onePieceApiImageCandidates(card) {
      ...
      return candidates.map(id => OPTCG_IMAGE_BASE_URL + encodeURIComponent(id));
        ...
        function migrateCardImageData(card) {
      if (!card || typeof card !== "object") return card;
      if (card.apiImage) return card;

      const enrichedCard = enrichCardForVariantImage(card);
      const derived = onePieceApiImageCandidates(enrichedCard).find(Boolean) || "";
      if (derived) {
        card.apiImage = derived;
      }
      return card;
    }
...
function openDetail(id) {
    ...
    function pickApiCard(apiCard) {
      ...
      // API-Bild wird nur als apiImage gespeichert und für die Galerie genutzt.
      // Es wird bewusst NICHT als eigenes hochgeladenes Foto/frontImage gesetzt,
      // damit die Detailansicht nur die echten hochgeladenen Kartenbilder zeigt.
