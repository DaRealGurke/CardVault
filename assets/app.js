...
<div class="select-card-check">${selectedCardIds.has(card.id) ? "✓" : ""}</div>
          <div class="card-image">${image ? `<img src="${image}" alt="${escapeHtml(card.name)}">` : "CARD"}
            <div class="psa-grade-label">
              <span class="psa-grade-number">${escapeHtml(card.grade || "-")}</span>
            </div>
          </div>
    ...
    function maybeAutoLoadCardmarketData() {
      setMarketAutoloadPill("Cardmarket: wartet auf Auto-Load…", "warning");
