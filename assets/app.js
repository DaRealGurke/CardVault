...
<div class="card-title-line">
    ...
    /* v221: Öffentliche Sammlung aus data/collection.json laden */
    const PUBLIC_COLLECTION_URL = "data/collection.json";
    ...
    async function loadPublicCollectionIfAvailable() {
      if (location.protocol === "file:") return false;
      try {
        const response = await fetch(PUBLIC_COLLECTION_URL + "?v=" + Date.now(), { cache: "no-store" });
        if (!response.ok) return false;

        const payload = await response.json();
        const publicCards = normalizePublicCollectionPayload(payload)
          .map(normalizePublicCard)
          .filter(card => card && card.name);

        if (!publicCards.length) return false;
        ...
        if (typeof showImportToast === "function") {
          showImportToast("Öffentliche Sammlung geladen", publicCards.length + " Karte(n) aus data/collection.json geladen.", "success");
        }
        return true;
      } catch (error) {
    ...
    function exportPublicCollectionJson() {
      ...
      URL.revokeObjectURL(url);
      if (typeof showImportToast === "function") {
        showImportToast("collection.json erstellt", "Diese Datei bei GitHub unter data/collection.json hochladen.", "success");
      }
    }
