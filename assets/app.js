    ...
    window.addEventListener("error", function(event) {
      ...
      if (box) {
        box.style.display = "block";
        box.textContent = "Fehler: " + event.message;
      }
    });
    ...
    function saveCards() {
      ...
      localStorage.setItem("cardVaultLastChange", new Date().toISOString());
      updateTrashButton();
...
refreshPreviews();
    ...
    function saveCard() {
      ...
      if (!editingId && existing) {
        showMessage("Diese Sammlungs-ID existiert bereits.", "error");
        return;
      }
      if (editingId && id !== editingId && existing) {
        showMessage("Diese Sammlungs-ID existiert bereits.", "error");
        return;
      }
      ...
      const card = {
        ...
        saleDate: $("saleDate") ? $("saleDate").value : "",
        platform: $("platform") ? $("platform").value.trim() : "",
    ...
    function updateQuickFilterButtons() {
      ...
      if ($("bulkFavoriteBtn")) onIfExists("bulkFavoriteBtn", "click", () => bulkUpdateSelected(card => { card.favorite = true; }));
      if ($("bulkSellBtn")) onIfExists("bulkSellBtn", "click", () => bulkUpdateSelected(card => { card.status = "Verkaufen"; }));
      if ($("bulkKeepBtn")) onIfExists("bulkKeepBtn", "click", () => bulkUpdateSelected(card => { card.status = "Behalten"; }));
    ...
    function updateBulkToolbar() {
      ...
      if (count) count.textContent = selectedCardIds.size + " ausgewählt";
...
<div class="select-card-check">${selectedCardIds.has(card.id) ? "✓" : ""}</div>
          <div class="card-image">${image ? `<img src="${image}" alt="${escapeHtml(card.name)}">` : "CARD"}</div>
          <div>
            <div class="tags">
              ${card.favorite ? `<span class="tag gold">★ Favorit</span>` : ""}
              <span class="tag green">In Sammlung</span>
              <span class="tag">${escapeHtml(card.status || "-")}</span>
              <span class="tag">${escapeHtml(card.language || "-")}</span>
              ${damageCount ? `<span class="tag">${damageCount} Markierung(en)</span>` : ""}
              ${(card.tags || []).slice(0, 4).map(tag => `<span class="tag">#${escapeHtml(tag)}</span>`).join("")}
              ${findMarketPrice(card) ? `<span class="tag">Cardmarket</span>` : ""}
            </div>
            <div class="card-title-line">
              <div class="card-title">${escapeHtml(card.name)}</div>
              <div class="mini-score">${escapeHtml(card.grade || "-")}</div>
            </div>
            ...
            <div class="details">
              ...
              <div><div class="detail-label">Gewinn</div><div class="detail-value">${formatProfit(card)}</div></div>
            </div>
            ${renderDocScore(card)}
    ...
    function saveCardmarketPrices() {
      const compact = cardmarketPrices.map(compactCardmarketPriceRecord);
      if (safeSetLargeLocalStorage("cardVaultCardmarketPrices", compact, "Preisverzeichnis")) {
        localStorage.setItem("cardVaultCardmarketPricesUpdated", new Date().toISOString());
      }
    }
    ...
    function saveCardmarketProducts() {
      const compact = cardmarketProducts.map(compactCardmarketProductRecord);
      if (safeSetLargeLocalStorage("cardVaultCardmarketProducts", compact, "Produktkatalog")) {
        localStorage.setItem("cardVaultCardmarketProductsUpdated", new Date().toISOString());
      }
    }
    ...
    function compactCardmarketProductRecord(record) {
      return {
        ...
        idExpansion: String(record.idExpansion || ""),
        idMetacard: String(record.idMetacard || ""),
        name: String(record.name || ""),
        baseName: String(record.baseName || record.name || ""),
    ...
    function renderMarketPriceDetail(card) {
      ...
      grid.innerHTML = `
        ...
        <div class="market-price-item"><div class="market-price-label">Product-ID</div><div class="market-price-value">${escapeHtml(price.idProduct || "-")}</div></div>
        <div class="market-price-item"><div class="market-price-label">Kategorie</div><div class="market-price-value">${escapeHtml(price.idCategory || "-")}</div></div>
      `;
    ...
    function catalogProductPrice(product) {
      ...
      return cardmarketPrices.find(price => String(price.idProduct) === String(product.idProduct)) || null;
    }
    ...
    function catalogProductMeta(product) {
      ...
      if (product.idExpansion) parts.push("Expansion " + product.idExpansion);
      if (product.idMetacard) parts.push("Meta " + product.idMetacard);
      return parts.join(" · ");
    }
    ...
    function renderCatalogSearchResults(targetId, results, mode = "full") {
      ...
      target.innerHTML = results.map(item => {
        ...
        return `
          <button class="catalog-result-card" type="button" data-catalog-pick="${escapeHtml(product.idProduct)}" data-catalog-mode="${escapeHtml(mode)}">
            <div>
            ...
            <div class="catalog-result-price">
              ...
              <span class="catalog-id-pill">ID ${escapeHtml(product.idProduct)} · Score ${item.score}</span>
            </div>
          </button>
        `;
      }).join("");
    ...
    function renderInlineCatalogSuggestions(results) {
      ...
      list.innerHTML = results.slice(0, 8).map(item => {
        ...
        const trend = price ? (price.trend || price.avg || price.avg30 || price.low) : "";
        return `
          <button class="inline-suggestion-card" type="button" data-inline-catalog-pick="${escapeHtml(product.idProduct)}">
            <div>
              <div class="inline-suggestion-name">${escapeHtml(catalogProductDisplayName(product))}</div>
              <div class="inline-suggestion-meta">${escapeHtml(catalogProductMeta(product))} · ID ${escapeHtml(product.idProduct)} · Score ${item.score}</div>
            </div>
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      document.querySelectorAll("[data-quick-filter]").forEach(button => {
        button.addEventListener("click", () => {
          ...
          if (activeQuickFilter === "all") $("favoriteFilter").value = "";
          renderCards();
        });
      });
