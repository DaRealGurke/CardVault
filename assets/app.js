...
refreshPreviews();
    ...
    function cardLooksLikeVariant(card) {
      ...
      return /(alt|alternate|alternative|parallel|manga|special|sp\b|sec\*|super parallel|wanted|treasure|comic|anniversary|winner|championship|promo)/i.test(text);
    }
    ...
    function onePieceApiImageCandidates(card) {
      ...
      if (variant) {
        for (let i = 1; i <= 12; i += 1) add(base + "_p" + i);
        for (let i = 1; i <= 6; i += 1) add(base + "_r" + i);
        for (let i = 1; i <= 3; i += 1) add(base + "_jp" + i);
        add(base);
      } else {
        add(base);
        for (let i = 1; i <= 6; i += 1) add(base + "_p" + i);
      }
    ...
    function galleryImageSources(card) {
      const apiSources = onePieceApiImageCandidates(card);
    ...
    function galleryImageHtml(card) {
      ...
      const apiBadge = first.includes("optcg-api") ? '<span class="api-image-badge">API</span>' : "";
      return `<img src="${escapeHtml(first)}" data-fallbacks="${rest}" data-own-front="${escapeHtml((card.images || [])[0] || "")}" alt="${escapeHtml(card.name || "Karte")}">${apiBadge}`;
    }
...
<div class="select-card-check">${selectedCardIds.has(card.id) ? "✓" : ""}</div>
          ...
          <div class="card-fixed-content">
            ...
            <div class="details">
              ...
              <div class="detail-grade-box"><div class="detail-label">Bewertung</div><div class="detail-grade-badge"><span class="detail-value detail-grade-value">${escapeHtml(card.grade || "-")}</span></div></div>
            </div>
            ${renderDocScore(card)}
